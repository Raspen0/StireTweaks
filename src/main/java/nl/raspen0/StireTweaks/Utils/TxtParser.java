	...
	public static ArrayList<String> parseFileMain(InputStream file)
	{
		...
		while (scanner.hasNextLine())
		{
			...
			if (temp.length() == 0 || temp.startsWith("**"))
			{
				...
				if (temp.startsWith("***"))
				{
					...
					if (validSkip)
					{
						...
						Strings.logger.error("StireTweaks - Invalid line-skip in changelog. This may not work as intended");
				}
			...
			else
			{
				...
				{
					if (nextPage.length() + temp.length() > 240)
					{
						bookText.add(nextPage);
						nextPage = temp;
						temp = "";
			...
			if (temp.length() == 0 || temp.startsWith("**"))
			{
				...
				if (temp.startsWith("***"))
				{
					...
					if (validSkip)
					{
						...
						Strings.logger.info("StireTweaks - Invalid line-skip in changelog. This may not work as intended");
				}
